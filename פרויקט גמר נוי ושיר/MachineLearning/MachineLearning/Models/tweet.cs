...
namespace WebApplication3.Models
{
    public class Tweet
    {
        ...
        public Tweet(string text, int sentiment)
        {
            this.text = text;
        ...
        public int Sentiment { get => sentiment; set => sentiment = value; }
        ...
        public void GetTweets()
        {
            ...
            List<Tweet> list = dbs.getList();
            List<Tweet> stemmedItems = new List<Tweet>();
            ...
            foreach (Tweet t in list)
            {
                ...
                {
                    StemmedText = StemmedText + StemWord(words[i]) + " ";
                }
                StemmedText = StemmedText.Remove(StemmedText.Length - 1);
                Tweet tweet = new Tweet(StemmedText, t.Sentiment);
                stemmedItems.Add(tweet);
            }
            Learn(stemmedItems);
        }


        public void Learn(List<Tweet> stemmedItems)
        {
            sumTextsInClasses = new int[3];
            foreach (Tweet item in stemmedItems)
            {
                sumTextsInClasses[item.Sentiment]++;
            }
            sumWordsInClasses = new int[3];
            uniqueWords = 0;//כמה מילים ייחודיות בכל הקלאסים ביחד
            countTexts = stemmedItems.Count;//כמה שורות טקסט בכל הקלאסים ביחד
            wordFreqInClasses = new Dictionary<string, int>[3];
            wordFreqInClasses[0] = new Dictionary<string, int>();
            wordFreqInClasses[1] = new Dictionary<string, int>();
            wordFreqInClasses[2] = new Dictionary<string, int>();
            foreach (Tweet item in stemmedItems)
            {
                string[] words = (item.Text).Split(' ');
                sumWordsInClasses[item.Sentiment] += words.Length;
                for (int w = 0; w < words.Length; w++)
                {
                    if (wordFreqInClasses[item.Sentiment].ContainsKey(words[w]))
                    {
                        wordFreqInClasses[item.Sentiment][words[w]] += 1;
                    }
                    else
                    {
                        wordFreqInClasses[item.Sentiment].Add(words[w], 1);
                    }
                }
                //n - grams(2)
                // Total number of n-grams we will have
                var total = words.Length - 2;
                ...
                for (int wo = 0; wo < grams.Count; wo++)
                {
                    if (wordFreqInClasses[item.Sentiment].ContainsKey(grams[wo]))
                    {
                        wordFreqInClasses[item.Sentiment][grams[wo]] += 1;
                    }
                    else
                    {
                        wordFreqInClasses[item.Sentiment].Add(grams[wo], 1);
                    }
                }
            }
            for (int i = 0; i < 3; i++)
            {
                uniqueWords += wordFreqInClasses[i].Count;
            }
        }

        public int Start(string text)
        {
            //split the text to 1 grams
            string[] words = text.Split(' ');
            for (int i = 0; i < words.Length; i++)
            {
                words[i] = StemWord(words[i]);
            }
            //split the text to 2 grams
            var total = words.Length - 2;
            List<string> grams = new List<string>();
            for (var i = 0; i <= total; i++)
            {
            ...
            double[] scoresOfClasses = new double[3];
            double maxClassScore = double.MinValue;
            //count a probability of being in class c for each class
            for (int c = 0; c < 3; c++)
            {
                currentClassScore = Math.Log(sumTextsInClasses[c] / (double)countTexts);//כמה טקסטים שליליים מתוך הסה"כ טקסטים
                for (int i = 0; i < words.Length; i++)
                {
                    if (words[i].Length <= 2) continue;//אם מדובר במילה של 2 אותיות ומטה לא מחשיבים אותה בניקוד
                ...
                {
                    wordFreqInClasses[c].TryGetValue(grams[i], out int w);
                    currentClassScore += Math.Log((w + 1) / (double)(uniqueWords + sumWordsInClasses[c]));
                }
